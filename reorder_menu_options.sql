...
-- =====================================================
-- SPECIFIC REORDERING REQUEST
-- =====================================================
-- Reorder main menu: id 1 becomes 3, id 2 becomes 1, id 3 becomes 2

-- Step 1: Temporarily set id 1 to a high number to avoid conflicts
UPDATE menu_nodes 
SET display_order = 999 
WHERE id = 1 AND parent_id IS NULL;

-- Step 2: Set id 2 to position 1
UPDATE menu_nodes 
SET display_order = 1 
WHERE id = 2 AND parent_id IS NULL;

-- Step 3: Set id 3 to position 2
UPDATE menu_nodes 
SET display_order = 2 
WHERE id = 3 AND parent_id IS NULL;

-- Step 4: Set id 1 to position 3
UPDATE menu_nodes 
SET display_order = 3 
WHERE id = 1 AND parent_id IS NULL;
